...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(self):
        ...
        self.physics = Box2D.b2World((0,0), contactListener=self.contactListener_keepref)
        self.viewer = None
    ...
    def _create_track(self):
        ...
        for i in range(len(track)):
            ...
            t = self.physics.CreateStaticBody( fixtures = fixtureDef(
    ...
    def step(self, actions, bodies):
        ...
        for body in bodies:
            body.step(1.0/FPS)
            x, y = body.hull.position
            if abs(x) > PLAYFIELD or abs(y) > PLAYFIELD:
                done = True

        self.physics.Step(1.0/FPS, 6*30, 2*30)
        self.t += 1.0/FPS
    ...
    def render(self, bodies, mode='human'):
        ...
        if mode=='human':
            ...
            for geom in self.viewer.onetime_geoms:
                geom.render()
            t.disable()
            self.render_indicators(WINDOW_W, WINDOW_H, bodies)
            win.flip()
    ...
    def render_indicators(self, W, H, bodies):
        gl.glBegin(gl.GL_QUADS)
        s = W/40.0
        h = H/40.0
